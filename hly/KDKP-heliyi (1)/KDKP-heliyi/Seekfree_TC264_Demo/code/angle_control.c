...
#define ENCODER_COUNTS_PER_RPM  (ANGLE_PPR / 60.0f)   // 1024 counts = 1 rev, 100ms period => counts_per_rpm = 1024/60

static int32 prev_count_for_speed = 0;
static uint8 speed_first_read = 1;
...
void angle_control_init(void) {
    ...
    angle_ctrl.speed_rpm = 0;

    // 初始化速度计算状态
    speed_first_read = 1;
    prev_count_for_speed = 0;
...
void angle_control_update(void) {
    ...
    // 计算电机转速 RPM: 100ms周期, 1024线编码器
    // RPM = (Δcount / 1024) * (60000 / 100) = Δcount * 600 / 1024
    if (speed_first_read) {
        prev_count_for_speed = current_count;
        angle_ctrl.speed_rpm = 0;
        speed_first_read = 0;
    } else {
        int32 delta = current_count - prev_count_for_speed;
        angle_ctrl.speed_rpm = (float)delta * 600.0f / (float)ANGLE_PPR;
        prev_count_for_speed = current_count;
    }
    ...
    // 限速：如果当前转速超过220 RPM，限制PWM输出
    if (angle_ctrl.speed_rpm > ANGLE_SPEED_MAX_RPM && pid_output > 0) {
        pid_output = 0;
    } else if (angle_ctrl.speed_rpm < -ANGLE_SPEED_MAX_RPM && pid_output < 0) {
        pid_output = 0;
    }

    angle_ctrl.pwm_output = (int32)pid_output;
    angle_motor_set_pwm(angle_ctrl.pwm_output);
