...
uint32                      gnss_rx_byte_count = 0;
uint32                      gnss_uart3_rx_isr_count = 0;
uint32                      gnss_uart3_er_isr_count = 0;
uint32                      gnss_gsv_count = 0;
uint8                       gnss_debug_line[GNSS_BUFFER_SIZE];
...
void gnss_uart_callback (void)
{
    ...
    if(gnss_state)
    {
        ...
        if('\n' == dat)
        {
            ...
            uint32 debug_length = fifo_used(&gnss_receiver_fifo);
            if(GNSS_BUFFER_SIZE <= debug_length)
            {
                debug_length = GNSS_BUFFER_SIZE - 1;
            }
            fifo_read_buffer(&gnss_receiver_fifo, gnss_debug_line, &debug_length, FIFO_READ_ONLY);
            gnss_debug_line[debug_length] = '\0';
            // 读取前6个数据 用于判断语句类型
            temp_length = 6;
            ...
            else if(0 == strncmp((char *)&temp_gps[3], "GSV", 3))
            {
                gnss_gsv_count++;
            }
