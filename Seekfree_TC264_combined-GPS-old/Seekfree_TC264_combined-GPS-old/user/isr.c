...
#include "code/angle_control.h"     /* 转向电机角度控制 */
...
/**
 * PIT Ch2 中断 (CCU61_CH0): 编码器速度采样 (10ms 周期, 100Hz)
 *
 * 每 10ms 读取双编码器脉冲数并清零, 供电机 PID 控制器使用
 * encoder_irq_update() 在 encoder.c 中实现
 */
IFX_INTERRUPT(cc61_pit_ch0_isr, 0, CCU6_1_CH0_ISR_PRIORITY)
{
    interrupt_global_enable(0);
    pit_clear_flag(CCU61_CH0);
    encoder_irq_update();               /* 编码器速度采样 */
    angle_control_update();             /* 转向电机角度控制 (10ms) */
}
 ...
 * UART0 接收中断: 无线串口数据接收
 *
 * 无线串口模块通过 UART0 (P14_0/P14_1) 以 115200 波特率通信
 */
...
/**
 * UART3 接收中断: GPS/GNSS 数据接收
 *
 * GPS 模块通过 UART3 (P15_6/P15_7) 以 9600 波特率发送 NMEA 数据帧
 */
IFX_INTERRUPT(uart3_rx_isr, 0, UART3_RX_INT_PRIO)
{
    interrupt_global_enable(0);

    gnss_uart3_rx_isr_count++;
    gnss_uart_callback();
}
