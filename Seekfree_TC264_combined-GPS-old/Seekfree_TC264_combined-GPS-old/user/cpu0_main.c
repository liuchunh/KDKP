 ...
 *   主循环 (约1ms):  IMU预测 + GPS更新 + 导航PID + 电机PID + 按键处理
 *
 * 修改记录:
...
#define INS_ONLY_MODE       0

#define WIRELESS_UART_MIN_TEST  0
...
#define MANUAL_MAX_SPEED_MPS (2.5f)
...
int core0_main(void)
{
...
#if WIRELESS_UART_MIN_TEST
    uart_init(UART_0, 115200, UART0_TX_P14_0, UART0_RX_P14_1);
    while (TRUE)
    {
        uart_write_string(UART_0, "\r\n[UART0 TEST] P14_0 TX 115200\r\n");
        system_delay_ms(500);
    }
#endif
    wireless_uart_init();
    uart_write_string(DEBUG_UART_INDEX, "\r\n[WIRELESS] UART0 P14_0/P14_1 115200 OK\r\n");
     ...
     * ================================================================ */
#if INS_ONLY_MODE
    uart_write_string(DEBUG_UART_INDEX, "[INIT] GPS SKIPPED (INS-only mode)\r\n");
#else
    uart_write_string(DEBUG_UART_INDEX, "[INIT] GPS GN42A init on UART3 P15_6/P15_7 9600...\r\n");
    gnss_init(GN42A);
    uart_write_string(DEBUG_UART_INDEX, "[INIT] GPS init done. Waiting for NMEA frames...\r\n");
#endif
...
#endif
        "========================================\r\n"
        "  S3 short: Record waypoint (打点)\r\n"
        "  S3 long:  Start/Stop nav (启停导航)\r\n"
        "  S4: Speed up (加速)\r\n"
        "  S5: Slow down (减速)\r\n"
        "  S6: Brake + Reset (刹车重置)\r\n"
        "  UART cmd: end + Enter = stop (串口停车)\r\n"
        "           <number> + Enter = set angle (串口设角度)\r\n"
        "========================================\r\n");
    ...
    uint32 gps_frame_count = 0;     /* GPS 帧计数 (用于检测 GPS 是否有数据) */
    uint32 imu_tick_count = 0;      /* IMU tick 计数 (用于验证 100Hz 采样) */

    /* ---- 串口命令解析缓冲区 ---- */
    uint8  cmd_buf[32];             /* 命令接收缓冲区 */
    uint8  cmd_len = 0;             /* 当前命令长度 */
    ...
    while (TRUE)
    {
        ...
        if (g_imu_tick)
        {
            ...
            if (g_nav_mode == NAV_RUNNING)
            {
                ...
                if (g_nav_mode == NAV_FINISHED) {
                ...
                } else if (g_nav_mode == NAV_PARKING) {
                    /* 到达停靠点 → 进入倒车入库 */
                    motor_control_stop();
                    servo_set_angle(0.0f);
                    uart_write_string(DEBUG_UART_INDEX,
                        "[NAV] Arrived at parking spot. Starting reverse parking...\r\n");
                } else {
                    /* 判断当前目标是否是车库相关航点 */
                    ...
                    motor_control_set_speed(target_speed);
            ...
            /* ---- 倒车入库控制 (NAV_PARKING 状态) ---- */
            else if (g_nav_mode == NAV_PARKING)
            {
                float park_speed, park_steer;
                nav_parking_update(&park_speed, &park_steer);

                if (park_speed != 0.0f) {
                    motor_control_set_speed(park_speed);    /* 倒车 (负速度) */
                } else {
                    motor_control_stop();
                }
                servo_set_angle(park_steer);
            }
            /* 非自动模式: 电机和舵机保持静止 */
            else if (g_nav_mode == NAV_IDLE && g_manual_speed > 0.0f)
            {
         ...
         *  S5 (减速):  目标速度 -0.2 m/s, 空闲时可减速到 0 (停车)
        ...
        if (!gpio_get_level(KEY_SPEED_UP))
        {
            ...
            if (!gpio_get_level(KEY_SPEED_UP))
            {
                ...
                if (g_speed_limit > MAX_SPEED_MPS) g_speed_limit = MAX_SPEED_MPS;
        ...
        if (!gpio_get_level(KEY_SLOW_DN))
        {
            ...
            {
                g_speed_limit -= MANUAL_SPEED_STEP;
                if (g_speed_limit < 0.0f) g_speed_limit = 0.0f;
        ...
        if (g_debug_tick)
        {
            ...
            if (debug_count >= 10)  /* 100ms x 10 = 1秒 */
            {
                ...
                /* ---- GPS 帧率 + 卫星数 (检测 GPS 是否正常工作) ---- */
                uart_write_string(DEBUG_UART_INDEX, "  GPS: frames/s=");
                uart_write_integer(DEBUG_UART_INDEX, gps_frame_count);
                uart_write_string(DEBUG_UART_INDEX, " sats=");
                uart_write_integer(DEBUG_UART_INDEX, gnss.satellite_used);
                uart_write_string(DEBUG_UART_INDEX, " fix=");
