...
// 1. 引入外部声明，让你能调用 ky_gnss.c 里的回调函数
extern void gnss_uart_callback(uint8_t dat);
...
void ASCLIN3_RX_ISR(void)
{
    ...
    /* 2. 直接读取硬件寄存器 RXDATA (修正拼写错误: RXDAT -> RXDATA)
     * 读取这个寄存器有两个作用：
     * A. 拿到数据
     * B. 硬件会自动清除中断标志位 <--- 解决卡死的关键
     */
    uint8_t dat = (uint8_t)MODULE_ASCLIN3.RXDATA.U;

    /* 3. 直接把数据塞给你的 GPS 处理函数 */
    gnss_uart_callback(dat);
}
