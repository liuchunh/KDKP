...
//#define ERU_CH0_CH4_INT_SERVICE IfxSrc_Tos_cpu0 //定义ERU通道0与通道4中断服务类型，即中断是由谁响应处理 0:CPU0 1:CPU1 3:DMA  不可设置为其他值
//#define ERU_CH0_CH4_INT_PRIO    40  //定义ERU通道0与通道4中断优先级 优先级范围1-255 越大优先级越高 与平时使用的单片机不一样
//
////通道1与通道5是公用一个中断函数 在中断内部通过标志位判断是谁触发的中断
//#define ERU_CH1_CH5_INT_SERVICE IfxSrc_Tos_cpu0 //定义ERU通道1与通道5中断服务类型，同上
//#define ERU_CH1_CH5_INT_PRIO    41  //定义ERU通道1与通道5中断优先级 同上
//
////通道2与通道6是公用一个中断函数 在中断内部通过标志位判断是谁触发的中断
//#define ERU_CH2_CH6_INT_SERVICE IfxSrc_Tos_dma  //定义ERU通道2与通道6中断服务类型，同上
//#define ERU_CH2_CH6_INT_PRIO    5   //定义ERU通道2与通道6中断优先级 同上
//
////通道3与通道7是公用一个中断函数 在中断内部通过标志位判断是谁触发的中断
//#define ERU_CH3_CH7_INT_SERVICE IfxSrc_Tos_cpu0 //定义ERU通道3与通道7中断服务类型，同上
//#define ERU_CH3_CH7_INT_PRIO    43  //定义ERU通道3与通道7中断优先级 同上
...
#define EXTI_CH3_CH7_INT_PRIO   254                 // 定义ERU通道3和通道7中断优先级 同上


//===================================================DMA中断参数相关定义===============================================
#define DMA_INT_SERVICE_1       IfxSrc_Tos_cpu0     // ERU触发DMA中断服务类型，即中断是由谁响应处理 IfxSrc_Tos_cpu0 IfxSrc_Tos_cpu1 IfxSrc_Tos_dma  不可设置为其他值
#define DMA_INT_PRIO_1          0                   // ERU触发DMA中断优先级 优先级范围1-255 越大优先级越高 与平时使用的单片机不一样
...
#define DMA_INT_PRIO_2          0                  // ERU触发DMA中断优先级 优先级范围1-255 越大优先级越高 与平时使用的单片机不一样
