...
enum TLDerr TLD7002initDevice(TLD7002_NetworkInstance_t *HSLInetwork, uint8 add )
{
    ...
    system_delay_us_register(tFAILSAFE2INIT); /* wait > FAILSAFE2INIT, so if the device it was in fail safe, it can move to init*/
...
boolean OTPwrite( uint16 *otp_cfg, TLD7002_NetworkInstance_t *HSLInetwork, uint8 address, uint16 newInterfrDelay  )
{
    ...
    system_delay_us_register(1005); /* delay counted starting from the last bit transmitted */  //TODO [ALL] magic number

    /* Step2: send command twice to syncronize LLD library master rolling counter and TLD7002 rolling counter */
    TLD7002_TX_PM_CHANGE_FRAME(HSLInetwork, (TLD7002_PM_CHANGE_FRAME_t *)gBuffTxRx, BROADCAST_ADDRESS, TLD7002_FRAME_PM_INIT_MODE);
    system_delay_us_register(1005);
    ...
    system_delay_us_register(INTERFR_DELAY_OTP_WRITE); /* delay counted starting from the last bit transmitted */
    ...
    system_delay_us_register(INTERFR_DELAY_OTP_WRITE);
    ...
    system_delay_us_register(max(newInterfrDelay, 205)); /* delay counted starting from the last bit transmitted, delay shall be:delay > 200us or bigger than the just configured interframe delay */
    ...
    system_delay_us_register(newInterfrDelay); /* wait > new programmed interframe delay */
    ...
    system_delay_us_register(newInterfrDelay); /* the interframe shall be counted starting from the last bit transmitted from the
    ...
    for (int n = 0; n < 5; n++)
    {
        ...
        system_delay_us_register (newInterfrDelay); /* wait interframe delay after TLD7002-16 answer */
    ...
    if (readBackMatch == TRUE)
        ...
        system_delay_us_register(2 * PWM_PERIOD_uS); /* wait for 2 PWM periods from the last HWCR command, CRC is checked 1 per PWM
...
boolean OTPemuComplete(uint16 *otp_cfg, TLD7002_NetworkInstance_t *HSLInetwork, uint8 add_after_emu, uint16 newInterfrDelay  )
{
    ...
    system_delay_us_register(INTERFR_DELAY_OTP_WRITE); //interframe delay
    ...
    system_delay_us_register(INTERFR_DELAY_OTP_WRITE);  /* wait for interframe delay */
     ...
     * configured by the new OTP array.*/
    TLD7002_TRX_PM_CHANGE(HSLInetwork, gBuffTxRx, BROADCAST_ADDRESS, TLD7002_FRAME_PM_INIT_MODE);
    system_delay_us_register(max(205, newInterfrDelay)); /*delay counted starting from the last bit transmitted,if the new
    ...
    system_delay_us_register(newInterfrDelay);
    ...
    for (int n = 0; n < 5; n++) /* entire CFC configurationis 40 words, read 5 times 8 word from the OTP (arduino has 62 bytes*/
    {
        ...
        system_delay_us_register (newInterfrDelay);/*wait interfframe delay after TLD7002-16 answer*/
 ...
 * system_delay_us_register(TLD7002_LEN_DC_UPDATE_8BIT_READ + TLD7002_LEN_DC_UPDATE_8BIT_WRITE )*TLD7002_BYTE_TIME_uS
 */
...
uint8 TLD7002_TRX_DC_UPDATE_8BIT(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, uint16 *dc_val)
{
    ...
    system_delay_us_register(TLD7002_LEN_DC_UPDATE_8BIT_READ * TLD7002_BYTE_TIME_uS); /* wait for TLD7002 reply */
...
uint8 TLD7002_TRX_DC_UPDATE_14BIT(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, uint16 *dc_val)
{
    ...
    system_delay_us_register(TLD7002_LEN_DC_UPDATE_14BIT_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available*/
...
uint8 TLD7002_TRX_READ_REG_DLC1(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, uint8 startAdd)
{
    ...
    system_delay_us_register(TLD7002_LEN_READ_REG_DLC1_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available */
...
uint8 TLD7002_TRX_READ_REG_DLC4(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, uint8 startAdd)
{
    ...
    system_delay_us_register( TLD7002_LEN_READ_REG_DLC4_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available */
...
uint8 TLD7002_TRX_DC_UPDATE_DLC0(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add)
{
    ...
    system_delay_us_register( TLD7002_LEN_DC_UPDATE_8BIT_DLC0_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available */
...
uint8 TLD7002_TRX_HWCR_ALL (TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add)
{
    ...
    system_delay_us_register( TLD7002_LEN_HWCR_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available*/
...
uint8 TLD7002_TRX_PM_CHANGE(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, TLD7002_FRAME_POWER_MODE_CHANGE_t mode)
{
    ...
    system_delay_us_register( TLD7002_LEN_PM_CHANGE_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available */
...
uint8 TLD7002_TRX_WRITE_REG_DLC7(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add, uint8 startAdd, uint16 *data)
{
    ...
    system_delay_us_register( TLD7002_LEN_WRITE_REG_READ * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available*/
...
uint8 TLD7002_TRX_READ_OST(TLD7002_NetworkInstance_t *mcldNet, char *gBuffTxRx, uint8 add)
{
    ...
    system_delay_us_register( TLD7002_LEN_READ_OST_WRITE * TLD7002_BYTE_TIME_uS ); /* wait > data from TLD7002-16 to be available*/
