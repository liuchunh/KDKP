...
/**
 * encoder_get_speed_mps() - 获取编码器实际速度 (m/s)
 *
 * 计算公式:
 *   rpm = (脉冲数 / PPR) * (60000 / 采样周期ms)
 *   speed_mps = rpm * 轮周长 / 60
 *
 * 需要根据实际车模调整:
 *   - WHEEL_DIAMETER: 驱动轮直径 (m)
 *   - GEAR_RATIO: 减速比 (电机转速 / 轮转速)
 */
#define WHEEL_DIAMETER  (0.065f)        /* 轮子直径 65mm */
#define WHEEL_CIRCUM    (WHEEL_DIAMETER * 3.14159f)  /* 轮周长 */
#define GEAR_RATIO      (30.0f)         /* 减速比 30:1 (根据实际修改) */

static float encoder_get_speed_mps(void)
{
    /* 编码器返回的是电机轴转速 (脉冲/采样周期) */
    /* 先转换为轮子转速，再转换为 m/s */
    float motor_rpm = encoder1_get_rpm();
    float wheel_rps = motor_rpm / GEAR_RATIO / 60.0f;  /* 轮子转/秒 */
    return wheel_rps * WHEEL_CIRCUM;
}

/**
 * motor_set_speed() - 速度闭环控制 (增量式PID + 编码器反馈)
 * @target_speed: 目标速度 (m/s), 正值前进, <=0 停车
 *
 * 实现原理: 编码器测速 → 增量式PID → PWM输出
 */
static void motor_set_speed(float target_speed)
{
    ...
    /* 读取编码器实际速度 */
    float current_speed = encoder_get_speed_mps();

    /* 增量式PID计算: 目标速度 → PWM占空比 */
    float pwm_duty = pid_compute(&g_speed_pid_params,
                                 &g_speed_pid_state,
                                 target_speed,
                                 current_speed);

    /* 输出限幅 */
    if (pwm_duty > (float)MOTOR_MAX_DUTY) pwm_duty = (float)MOTOR_MAX_DUTY;
    if (pwm_duty < 0.0f) pwm_duty = 0.0f;

    pwm_set_duty(MOTOR_PWM_CH, (uint32)pwm_duty);
}
...
{
    /* ---- 速度 PID (增量式) ---- */
    /* 增量式PID适合电机控制，参数不宜太超调 */
    pid_init(&g_speed_pid_params,
             3.0f,               /* Kp: 比例系数 */
             0.5f,               /* Ki: 积分系数 */
             0.1f,               /* Kd: 微分系数 */
             0.01f,              /* dt: 控制周期 10ms */
             0.0f,               /* output_min: 最低PWM 0 */
             (float)MOTOR_MAX_DUTY,  /* output_max: 最大PWM */
             PID_INCREMENT);     /* 增量式 PID */
    pid_set_dead_zone(&g_speed_pid_params, 0.05f);  /* 死区 0.05m/s */
    pid_reset(&g_speed_pid_state);
    ...
    while (TRUE)
    {
        ...
        if (g_debug_tick)
        {
            ...
            if (debug_count >= 10)  /* 100ms x 10 = 1秒 */
            {
                ...
                /* 编码器速度 */
                float enc_speed = encoder_get_speed_mps();
                int16 enc_pulse = encoder1_get_speed();
                uart_write_string(DEBUG_UART_INDEX, "  Enc: ");
                uart_write_float(DEBUG_UART_INDEX, enc_speed);
                uart_write_string(DEBUG_UART_INDEX, "m/s (");
                uart_write_integer(DEBUG_UART_INDEX, enc_pulse);
                uart_write_string(DEBUG_UART_INDEX, " pulse)\r\n");
