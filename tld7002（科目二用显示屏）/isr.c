...
extern void uart_rx_callback(void);     // cpu0_main.c 中定义的串口接收回调
...
IFX_INTERRUPT(exti_ch3_ch7_isr, 0, EXTI_CH3_CH7_INT_PRIO)
{
    ...
    if(exti_flag_get(ERU_CH7_REQ11_P20_9))          // 通道7中断
    {
        ...
        dot_matrix_screen_scan();                   // 点阵屏扫描刷新
    }
}
// **************************** 外部中断函数 ****************************
...
{
    interrupt_global_enable(0);                     // 开启中断嵌套
    uart_rx_callback();                             // 读取串口字节写入 FIFO
...
{
    interrupt_global_enable(0);                     // 开启中断嵌套
    tld7002_callback();                             // TLD7002 串口接收回调函数
}
